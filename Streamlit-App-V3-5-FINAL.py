"""
PetroLens Executive v3.5 PRODUCTION - FINAL
...
st.set_page_config(page_title="🛢️ PetroLens Executive v3.5 Production", layout="wide", page_icon="🛢️")

# ----------------- PRODUCTION CONFIG - HARDCODED, NO INPUT -----------------
API_URL = "https://geoscience-doc-copilot.onrender.com/api/upload-report"
# For Cloud Run: change to your Cloud Run URL: https://YOUR-SERVICE-xxxx-uc.a.run.app/api/upload-report
...
with st.sidebar:
    ...
    st.caption("Build: 2026-09-24 • v3.5 Production")

st.title("🛢️ PetroLens - Subsurface Intelligence")
st.caption("v3.5 Production | Commercial Viability Gauge | POSg Multiplicative | Executive Reporting")

uploaded_file = st.file_uploader("Upload Geoscience Report (PDF/DOCX/TXT/MD)", type=["pdf","docx","txt","md"])
deep = st.checkbox("Deep Analysis (gemini-3.1-pro-preview)", value=False, help="Pro model requires billing (0 free quota). Unchecked uses Flash (20/day free).")
...
if uploaded_file and st.button("🚀 Generate Executive Analysis", type="primary", use_container_width=True):
    # --- FIXED: Spinner only wraps API call, not error handling ---
    resp = None
    api_error = None
    ...
    with st.spinner("Running petroleum system evaluation... (cold start 50s + analysis 30s). Please wait..."):
        try:
            ...
            api_error = ("timeout", str(e))
        except requests.exceptions.ConnectionError as e:
            api_error = ("connection", str(e))
        except Exception as e:
            api_error = ("general", str(e))

    # --- Error handling OUTSIDE spinner so spinner always stops ---
    if api_error:
        kind, msg = api_error
        if kind == "timeout":
            st.error("⏱ Request timed out (cold start >100s)")
            st.markdown(f"1. Open **{API_URL.replace('/api/upload-report','')}** in new tab → Wait 30s\n2. Click Generate again\n3. Add UptimeRobot ping every 5 min")
        elif kind == "connection":
            st.error("🔌 Cannot reach backend")
            st.code(msg[:500])
        else:
            st.error(f"❌ Error: {msg[:800]}")
        st.stop()

    # Now handle HTTP codes outside spinner
    if resp is None:
    ...
    if resp.status_code == 429:
        try:
            err_detail = resp.json().get("detail", resp.text[:800])
        except:
            err_detail = resp.text[:800]
        st.error("🚫 Quota Exceeded - 429")
        st.code(err_detail)
        st.warning("Free tier: 20/day. Wait 60s, create new project, or enable billing. Resets 8am Lagos.")
        st.stop()
    ...
    if resp.status_code == 503:
        try:
            ...
            err_detail = "Google preview models overloaded (temporary 1-2 mins)"
        st.error("🔥 Service temporarily overloaded (503)")
        st.code(err_detail)
        st.info("Wait 60 seconds and click Generate Executive Analysis again. No quota used. Preview models spike during peak hours.")
        st.stop()
    ...
    if resp.status_code == 404:
        try:
            ...
            err_detail = resp.text[:800]
        st.error("❌ Model not available (404/426) - Sep 2026 keys only allow gemini-3-*")
        st.code(err_detail)
        st.stop()
