...
try:
    ...
    col_gauge1, col_gauge2, col_verdict = st.columns([2,2,1])
    with col_gauge1:
        fig_gauge1 = go.Figure(go.Indicator(mode="gauge+number", value=posg*100, title={'text': "POSg - Geological CoS"}, number={'suffix': "%"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "#10b981" if posg>=0.25 else "#ef4444"}, 'steps': [{'range': [0, 25], 'color': "#fee2e2"}, {'range': [25, 50], 'color': "#fef3c7"}, {'range': [50, 100], 'color': "#dcfce7"}]}))
        fig_gauge1.update_layout(height=250, margin=dict(l=20,r=20,t=40,b=20))
        st.plotly_chart(fig_gauge1, use_container_width=True)
        st.caption(f"{' × '.join([f'{k}({v:.2f})' for k,v in breakdown.items()])} = {posg:.3f}")
    with col_gauge2:
        viability = float(a.get('commercial_viability_score', 0))
        fig_gauge2 = go.Figure(go.Indicator(mode="gauge+number", value=viability, title={'text': "Viability Score"}, number={'suffix': "/100"}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "#f59e0b"}, 'steps': [{'range': [0, 50], 'color': "#fee2e2"}, {'range': [50, 70], 'color': "#fef3c7"}, {'range': [70, 100], 'color': "#dcfce7"}]}))
    ...
    st.subheader("Petroleum System Confidence")
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(x=list(elements.keys()), y=[v["score"] for v in elements.values()], marker=dict(color=["#1e40af","#3b82f6","#60a5fa","#93c5fd","#bfdbfe"]), width=0.35, text=[f"{v['score']}" for v in elements.values()], textposition='outside', hovertemplate="%{x}: %{y}/100<br>Prob: %{customdata:.2f}<extra></extra>", customdata=[v["prob"] for v in elements.values()]))
    ...
    st.divider()
    pdf_buffer = create_executive_pdf(a, uploaded_file.name, posg_data, model_used)
    st.download_button(label="Download Executive Report PDF", data=pdf_buffer, file_name=f"PetroLens_Executive_v3_5_{uploaded_file.name.split('.')[0]}_POSg{int(posg*100)}.pdf", mime="application/pdf", type="primary", use_container_width=True)
    st.caption(f"Model: {model_used} | POSg: {posg*100:.1f}% | File: {uploaded_file.name}")
except Exception as e:
    st.error(f"Error during analysis: {e}")
    st.exception(e)
