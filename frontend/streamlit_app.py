...
st.set_page_config(page_title="PetroLens Executive v3.1", layout="wide", page_icon="🛢️")

# ----------------- CORE CALCULATIONS (REAL) -----------------
def calc_posg(probs_dict):
    """Multiplicative POSg: P = P_src * P_res * P_trap * P_seal * P_charge"""
    p = 1.0
...
def normalize_elements(raw_breakdown):
    """
    Takes backend viability_breakdown or petroleum_system and returns normalized 0-100 capped elements
    Expected raw_breakdown keys: source_rock, reservoir_quality, trap_seal, etc.
    """
    # If backend already returns petroleum_system with score/prob/comment, use it
    ...
    # Fallback: map viability_breakdown (old format) to 5 elements capped 100
    bd = raw_breakdown or {}
    elements = {
...
def create_executive_pdf(analysis, filename, posg_data, model_used):
    ...
    story.append(Paragraph("PetroLens AI - Commercial Viability Report v3.1", title_style))
    ...
    story.append(Paragraph("Executive Summary (Real Gemini Output)", h2))
    ...
    story.append(Paragraph("Risk Factors (Real)", h2))
...
if uploaded_file and st.button("Generate Executive Analysis", type="primary", use_container_width=True):
    with st.spinner("Running senior geologist evaluation... (Render free tier cold start 50s + Gemini 30s = ~80s). Please wait..."):
        try:
            ...
            try:
                ...
                **Real fix:**
                ...
                2. Come back and click **Generate Executive Analysis** again (don't refresh, just click)
                3. For defense, add **UptimeRobot.com** ping every 5 min to keep Render awake (free)
                """)
                st.stop()
            except requests.exceptions.ConnectionError as ce:
                st.error(f"🔌 ConnectionError: Cannot reach API at {api_url}")
                st.markdown("**Check:** 1) Render service is Live? 2) API URL correct? 3) Render sleeping - open root URL to wake.")
                st.code(str(ce)[:500])
                st.stop()

            # REAL QUOTA HANDLING - No mock data, real error messages
            if resp.status_code == 429:
                ...
                **Real fixes (no fake):**
                ...
                2. **Create NEW PROJECT** at https://aistudio.google.com/app/apikey → Top left → New Project → Create Key in it → Update Render env + Manual Deploy → New project = real 20 free
                3. **Enable billing** at https://ai.google.dev/gemini-api/docs/billing (free $300 credit) → Limit becomes 1500/day real
                4. **Switch model to gemini-2.0-flash-lite** in backend (30 RPM, 1500 RPD free - 75x more) for real results
                5. **Resets daily at 8am Lagos** (midnight PT) - real reset, not fake
                
                **Current API stats:** Check https://ai.dev/rate-limit → Select your project → See 20/20 vs 0/20
                """)
                st.stop()
            
            if resp.status_code == 503:
                st.error("🔥 **503 Service Unavailable - Gemini High Demand (real Google overload)**")
                st.info("**Real fix:** Wait 30-60 seconds and click Generate again. No fake data will be shown. This is Google's side, not your code.")
                st.stop()
            ...
            model_used = result.get("model_used", "gemini-3-flash-preview (real)")
            
            # Build petroleum system elements (real from backend)
            ...
            # Ensure 0-100 capped and 0.05-0.99 prob
            for k in elements:
            ...
            with col_gauge1:
                fig_gauge1 = go.Figure(go.Indicator(
                    mode="gauge+number", 
                    value=posg*100, 
                    title={'text': "POSg - Geological CoS (Real)", 'font': {'size': 14}}, 
                ...
                st.caption(f"Formula (real): {' × '.join([f'{k}({v:.2f})' for k,v in breakdown.items()])} = {posg:.3f}")
            ...
            with col_gauge2:
                ...
                fig_gauge2 = go.Figure(go.Indicator(
                    mode="gauge+number", 
                    value=viability, 
                    title={'text': "Viability Score (Real)", 'font': {'size': 14}}, 
            ...
            with col_verdict:
                st.metric("Verdict (Real)", a.get('commercial_verdict', 'APPRAISE'))
                st.metric("AFC Flag (Real)", a.get('AFC_flag', 'Med'))
            ...
            fig2.add_trace(go.Bar(
                ...
                y=[v["score"] for v in elements.values()], 
                marker=dict(color=["#f59e0b","#fbbf24","#fde68a","#fb923c","#facc15"]), 
                width=0.25, 
                text=[f"{v['score']}/100" for v in elements.values()], 
            ...
            st.markdown("**Key Phrases Commentary (Real from report)**")
            ...
            with c1:
                st.markdown(f"""
                - **Dominant Fluid:** {fluid.get('dominant_fluid', a.get('dominant_fluid','-'))}
                - **Oil Quality:** {fluid.get('oil_quality','-')}
                - **Gas Quality:** {fluid.get('gas_quality','-')}
            ...
            with c2:
                st.markdown(f"""
                - **STOIIP:** {vol.get('stoiip', a.get('stoiip','-'))}
            ...
            # ----------------- PDF DOWNLOAD (REAL DATA) -----------------
            pdf_buffer = create_executive_pdf(a, uploaded_file.name, posg_data, model_used)
            st.download_button(
                label="📄 Download Executive Report PDF v3.1 (Real Data)", 
                data=pdf_buffer, 
                file_name=f"PetroLens_Executive_v3_1_{uploaded_file.name.split('.')[0]}_POSg{int(posg*100)}_Real.pdf", 
                mime="application/pdf", 
                type="primary", 
                use_container_width=True
            )
            st.caption(f"Model used: {model_used} | POSg: {posg*100:.1f}% | File: {uploaded_file.name} | Real output, no mock")

        except Exception as e:
            st.error(f"❌ **Real error during analysis (no fake fallback):** {e}")
            st.exception(e)
            st.info("This is real error trace - no hallucinated data shown. Check backend logs at https://dashboard.render.com/")
